...
public class ExhaustiveGrammarExplorer extends AbstractGrammarExplorer {
	...
	public void startExploration(int startNoOfNonTerminals,int endNoOfNonTerminals, int startRulesAt, int nBestGrammarsToKeep, Dataset fullDataset,
		...
		while(startNoOfNonTerminals<=endNoOfNonTerminals){

			Map<Integer,Rule> integerRuleMap= new AutoGeneratedGrammars(nNonterminals).generateRules();
			...
			for (int k = startRulesAt; k <= noOfRules; k++) {
				...
				while (SSI.hasNext()) {
					...
					grammarIndex++;
					SecondaryStructureGrammar ssg = getNextGrammar(startNoOfNonTerminals,k,subArray,grammarIndex,integerRuleMap);
