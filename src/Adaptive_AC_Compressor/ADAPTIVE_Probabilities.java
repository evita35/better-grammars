 ...
 * @author Eva
 */
public class ADAPTIVE_Probabilities {
    
    String PrimarySeq;//stores RNA primary sequency
    String DotBracket;    //stores RNA secondary structure
    int[] count; //stores the frequency of occurence of all the events in the RNA string
    ADAPTIVE_Production_Rule_Frequency PRF;
     ...
     ADAPTIVE_Probabilities(String PrySeq, String DotBrack)
     { 
         PrimarySeq=PrySeq; DotBracket=DotBrack;
         PRF = new ADAPTIVE_Production_Rule_Frequency();
         //System.out.printf("%s\n %s\n", PrySeq, DotBrack);
         count = PRF.getCount();//initialises the counter array which stores frequency of each event in the RNA string
        // printArray(count);
         setProbabilities();//constructor calls method to initialise probabilities
         //printProbabilities();
     }
     public void printCount(){
         printArray(count);
     }
     public void setCount(String nucleotide){
         count = PRF.setCount(nucleotide);
         setProbabilities();
     }
     
     public void setCount(int index){
         count = PRF.setCount(index);
         setProbabilities();
     }
    
    public void setProbabilities(){
                
        SProbs[0]= (double)(count[10]/(double)(count[10]+count[11]));//initialising SProbs for the event S->LS
    ...
    void printArray(double[] integerArray){//displays the contents of the array
