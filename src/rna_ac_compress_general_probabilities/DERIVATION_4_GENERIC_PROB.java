 ...
 * @author EVA
 */
public class DERIVATION_4_GENERIC_PROB {
    /**RNA grammar rules
     * S-> LS|e
    ...
    static BigDecimal[] Interval= {BigDecimal.ZERO, BigDecimal.ONE};//LOWER BOUND IS INITIALISED
    boolean DEBUG = false; //variable is set to false initially, but set to true for debugging purposes
    
    GENERIC_Probabilities RNAProb = new  GENERIC_Probabilities();//creates object of the class
    
    double[] LProbs1=RNAProb.getLProbs();//LProbs1 contains the probabilities of occurence of the events
                                        // L-> aSu | uSa | cSg | gSc | uSg | gSu | a| u| c| g
    double[] sumLProbs1=RNAProb.getSumLProbs();//sumLProbs1 contains the boundaries of the intervals of the events
                                        // L-> aSu | uSa | cSg | gSc | uSg | gSu | a| u| c| g
    double[] SProbs1 = RNAProb.getSProbs();//SProbs1 contains the probabilities of occurence of the events
                                        // S->LS and S->e
    double[] sumSProbs1=RNAProb.getSumSProbs();//sumLProbs1 contains the boundaries of the intervals of the events
                                        //S->LS and S->e
